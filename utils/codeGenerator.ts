  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-900">
      <div className="p-8 rounded-2xl shadow bg-white">
        ...
        <p className="mt-2 text-slate-600">The project skeleton has been initialized.</p>
      </div>
    </div>
  );
}
...
const removeTailwindDirectives = (content: string): string => {
    return content.split('\n').filter(line => !line.trim().startsWith('@tailwind')).join('\n');
};

export const sanitizeFileContent = (content: string, path: string): string => {
    if (!content) return "";
    let clean = stripMarkdownFences(content);
    if (clean.includes('\\n') || clean.includes('\\"')) clean = decodeEscapes(clean);
    if (path.match(/\.(tsx|jsx|ts|js|html|css|json|md)$/)) {
        if (clean.includes('&lt;') || clean.includes('&gt;') || clean.includes('&amp;')) {
            clean = clean.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&amp;/g, '&').replace(/&quot;/g, '"').replace(/&#39;/g, "'");
        }
    }
    if (path.endsWith('.css')) clean = removeTailwindDirectives(clean);
    return clean.trim();
};

export const validateProjectSafety = (files: ProjectFile[]): string | null => {
    for (const f of files) {
        if (f.content.includes('```')) return `Sanitization Failed: File '${f.path}' still contains Markdown fences.`;
        if (f.content.includes('[object Object]')) return `Sanitization Failed: File '${f.path}' contains '[object Object]'.`;
    }
    return null;
};
...
export const normalizeFiles = (files: ProjectFile[]): { files: ProjectFile[], entryPoint: string | null } => {
    ...
    files.forEach(f => {
        ...
        if ((path.endsWith('.tsx') || path.endsWith('.ts') || path.endsWith('.css')) && 
            !path.startsWith('src/') && !path.includes('config') && !path.includes('.d.ts')) {
            path = `src/${path}`;
        }
    ...
    // Force React Entry Point
    if (fileMap.has('src/main.tsx')) entryPoint = 'src/main.tsx';
    else if (fileMap.has('src/index.tsx')) entryPoint = 'src/index.tsx';
    else {
        entryPoint = 'src/main.tsx';
        fileMap.set(entryPoint, { path: entryPoint, content: DEFAULT_MAIN_TSX, type: 'file', language: 'typescript' });
    }
    ...
    // Ensure App.tsx exists for entryPoint
    ...
    if (!fileMap.has('src/index.css')) {
        fileMap.set('src/index.css', { path: 'src/index.css', content: DEFAULT_INDEX_CSS, type: 'file', language: 'css' });
    }
...
export const constructMultiFileDocument = (rawFiles: ProjectFile[], projectId?: string, project?: Project | null): string => {
  ...
  if (safetyError) return `<!DOCTYPE html><html><body style="background:#0f172a;color:#ef4444;display:flex;align-items:center;justify-content:center;height:100vh;padding:2rem;"><div style="text-align:center"><h3>Security Block</h3><pre>${safetyError}</pre></div></body></html>`;

  const indexHtml = processedFiles.find(f => f.path === 'index.html')!;
  let baseHtmlContent = indexHtml.content;
  if (!baseHtmlContent.toLowerCase().includes('<html')) baseHtmlContent = `<!DOCTYPE html><html>${baseHtmlContent}</html>`;
  else if (!baseHtmlContent.includes('<!DOCTYPE html')) baseHtmlContent = `<!DOCTYPE html>${baseHtmlContent}`;
  ...
  if (project?.rafieiCloudProject?.status === 'ACTIVE' && project.rafieiCloudProject.projectRef && project.rafieiCloudProject.publishableKey) {
      ...
      envInjection = `window.process = { env: { NODE_ENV: 'development', SUPABASE_URL: '${url}', SUPABASE_ANON_KEY: '${key}', VITE_SUPABASE_URL: '${url}', VITE_SUPABASE_ANON_KEY: '${key}', NEXT_PUBLIC_SUPABASE_URL: '${url}', NEXT_PUBLIC_SUPABASE_ANON_KEY: '${key}' } };`;
  }
  ...
  baseHtmlContent = baseHtmlContent.replace('</head>', `<style>${styles}</style>\n</head>`);
  const safeFileMapJson = safeJsonStringify(fileMap);
  const entryScript = entryPoint ? `__require('${entryPoint}');` : '';

  const injectedScripts = `
    <script>${envInjection}\n${projectIdInjection}\nwindow.onerror = function(msg, url, line, col, error) { window.parent.postMessage({ type: 'RUNTIME_ERROR', message: msg + (error ? '\\n' + error.stack : '') }, '*'); };</script>
    ...
    <script src="https://cdn.tailwindcss.com"></script>
    <script>
      tailwind.config = { theme: { extend: { fontFamily: { sans: ['Vazirmatn', 'Inter', 'sans-serif'] } } } }
    </script>
    <script>
      ...
      function __require(path, base = 'src/main.tsx') {
          ...
          if (cleanPath === 'react-dom' || cleanPath === 'react-dom/client') return window.ReactDOM;
          if (cleanPath === 'react-router-dom') {
              const lib = window.ReactRouterDOM;
              return lib ? { ...lib, BrowserRouter: lib.HashRouter } : {};
          }
          if (cleanPath === '@supabase/supabase-js') return window.supabase;
          if (cleanPath === 'lucide-react' || cleanPath.startsWith('lucide-react/')) return window.lucideReact || window.lucide;
          ...
          const module = { exports: {} }; window.__MODULES__[finalPath] = module;
          if (finalPath.endsWith('.css')) return {};
          if (finalPath.endsWith('.json')) { try { module.exports = JSON.parse(source); } catch(e) { module.exports = {}; } return module.exports; }
          try {
